...
function Dashboard({ goChat }: { goChat: () => void }) {
  ...
  return <div className="min-h-[852px] bg-buddy-bg text-buddy-text"><StatusBar /><div className="flex h-[804px] flex-col px-5 pb-4 pt-4"><div className="flex items-center justify-between"><div><h2 className="font-display text-2xl font-extrabold">FitBuddy</h2><p className="text-sm text-buddy-muted">Plan activo · Semana 1</p></div><div className="flex items-center gap-3"><button className="flex h-11 w-11 items-center justify-center rounded-full bg-white shadow-sm ring-1 ring-buddy-border"><Bell className="h-5 w-5" /></button><BuddyMascot size="sm" /></div></div><div className="mt-5 flex rounded-2xl bg-[#EEF2FB] p-1 text-xs font-extrabold text-buddy-muted">{[['home','Inicio'],['plan','Plan'],['memory','Memoria'],['settings','Ajustes']].map(([id,label]) => <button key={id} onClick={() => setTab(id)} className={cn('flex-1 rounded-xl py-2', tab === id ? 'bg-white text-buddy-purple shadow-sm' : '')}>{label}</button>)}</div><div className="mt-5 flex-1 overflow-auto">{tab === 'home' && <DashboardHome />}{tab === 'plan' && <PlanTab />}{tab === 'memory' && <MemoryTab />}{tab === 'settings' && <SettingsTab />}</div><div className="mt-4 grid grid-cols-4 rounded-[28px] border border-buddy-border bg-white p-2 shadow-buddyFloating">{[['home','Inicio',Home],['chat','Chat',MessageCircle],['memory','Memoria',Sparkles],['settings','Ajustes',Settings]].map(([id,label,Icon]) => <button key={String(id)} onClick={id === 'chat' ? goChat : () => setTab(String(id))} className={cn('flex flex-col items-center gap-1 rounded-2xl py-2 text-[11px] font-extrabold', tab === id ? 'bg-buddy-purple text-white' : 'text-buddy-muted')}><Icon className="h-5 w-5" />{label}</button>)}</div></div></div>;
}
